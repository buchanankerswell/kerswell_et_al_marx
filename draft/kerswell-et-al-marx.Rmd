...
> 1. Recovered markers from most individual numerical experiments show discrete multimodal PT distributions with steep step-like CDFs (Figure \@ref(fig:bigComp) & Supporting Information)
> 
> 2. While relatively few markers are recovered from PT regions coinciding with high-frequencies of natural samples around 2 GPa and 550 ˚C, markers and natural samples recovered from near 1 GPa show moderate overlap within typical uncertainties for thermobarometry and phase equilibrium modeling (50–100 ˚C; compare Figures \@ref(fig:marxComp), \@ref(fig:tkSummaryYoung), & \@ref(fig:tkSummaryOld))
> 
> 3. Markers are frequently recovered from a major P mode near 1 GPa and very infrequently recovered  between 2–2.5 GPa with a higher rate of recovery from lower P's (`r round(cdfP.summary$prob[which.min(abs(cdfP.summary$P-1.5))]*100)`% from ≤ 1.5 GPa) compared to natural samples (36-59% from ≤ 1.5 GPa; compare Figures \@ref(fig:marxComp), \@ref(fig:tkSummaryYoung), & \@ref(fig:tkSummaryOld))
> 
> 4. Markers are frequently recovered from a major T mode near 275 ˚C and very infrequently recovered between 400–600 ˚C with a higher rate of recovery from lower T's (`r round(cdfT.summary$prob[which.min(abs(cdfT.summary$T-525))]*100)`% from ≤ 525 ˚C) compared to natural samples (44-50% from ≤ 525 ˚C; compare Figures \@ref(fig:marxComp), \@ref(fig:tkSummaryYoung), & \@ref(fig:tkSummaryOld))
> 
> 5. The relative abundance of markers recovered along "typical" thermal gradients for subduction zones (`r round(cdfGrad.summary$prob[which.min(abs(cdfGrad.summary$grad-10))]*100)-round(cdfGrad.summary$prob[which.min(abs(cdfGrad.summary$grad-5))]*100)`% from 5-12 ˚C/km) is high compared to natural samples (59-78% from 5-12 ˚C/km; compare Figures \@ref(fig:marxComp), \@ref(fig:tkSummaryYoung), & \@ref(fig:tkSummaryOld))
> 
> 6. Many markers are recovered from the forbidden zone (`r round(cdfGrad.summary$prob[which.min(abs(cdfGrad.summary$grad-5))]*100)`% from ≤ 5 ˚C/km; compare Figures \@ref(fig:marxComp), \@ref(fig:tkSummaryYoung), & \@ref(fig:tkSummaryOld))
> 
> 7. Virtually no markers (`r round((1-cdfGrad.summary$prob[which.min(abs(cdfGrad.summary$grad-15))])*100, 3)`%) are recovered from ≥ 15 ˚C/km compared to natural samples (6-30% from ≥ 15 ˚C/km; compare Figures \@ref(fig:marxComp), \@ref(fig:tkSummaryYoung), & \@ref(fig:tkSummaryOld))
...
The termination of the low-velocity zone at depths beyond the continental Moho marks another important mechanical transition. This second transition is often interpreted as the onset of mechanical plate coupling near 80 km [or 2.3 GPa; @furukawa1993; @wada2009; @agard2009; @agard2018] and coincides well with the deeper recovery modes (P mode2) of markers at `r round(mean(stats.summary$mode2.P.est), 1)` $\pm$ `r round(2*sd(stats.summary$mode2.P.est), 1)` GPa (2 $\sigma$, Table \@ref(tab:recSummary)). Although these deeper modes of marker recovery from near 2 GPa are noticeable in our numerical experiments (and consistent with maximum recovery depths implied in pd15 and ag18), they represent a minor fraction of recovered markers (typically $\leq$ 1–10%), which is inconsistent with the amount of material represented in pd15 and ag18 that is estimated to be recovered from near 2 GPa. Between the major mode of recovery at $\sim$ 40 and the relatively few markers recovered from maximum depths near $\sim$ 80 km lies a gap in marker recovery that coincides with the highest sample frequencies of exhumed HP rocks compiled in pd15 and ag18 (Figure \@ref(fig:marxComp)). This recovery gap is discussed in the following section.
...
> 
> 4. Rocks are recovered during short-lived events [e.g. subduction of seamounts, @agard2009] that are not implemented in our numerical experiments, rather than recovered during steady-state subduction within a serpentine-rich tectonic mélange that is characteristic of our numerical experiments (i.e., geodynamic uncertainties, Section  \@ref(geodynamicUncertainties))
...
> 1. Numerical simulations with relatively simple (de)hydration models and visco-plastic interface rheologies simulate important recovery mechanisms near the base of the continental Moho around 1 GPa and 275 ˚C (underplating and/or formation of tectonic mélanges) and near the depth of mechanical plate coupling around 2.5 GPa and 525 ˚C.
> 
...
> 
> 3. A gap in marker recovery near 2 GPa and 550 ˚C coinciding with the highest frequencies of natural samples suggests an "overabundance" of samples are studied from this PT region. Explanations for this "overabundance" might include selective sampling of rocks amenable to petrologic investigation (scientific bias), reaction overstepping (abrupt release of water triggering detachment of rock near 2 GPa and 550 ˚C), or processes such as subduction of seamounts that are not included in numerical simulations. Future work investigating natural samples from a larger range of peak PT conditions and analyzing marker recovery from numerical geodynamic models that include new specific implementations of metamorphic (de)hydration reactions and interface rheologies might help resolve this discrepancy.
